...
class NaverLandCollector:
    ...
    def _warmup_session(self) -> None:
        if self._warmed_up:
            return
        req = Request(url=f"{BASE_URL}/complexes", headers=self._build_headers(referer=f"{BASE_URL}/"))
        try:
            with self.opener.open(req, timeout=self.timeout):
                self._warmed_up = True
        except Exception:
            # 워밍업 실패는 치명적이지 않으므로 본 요청에서 재시도.
            pass

    def _compute_retry_wait(self, http_error: HTTPError, attempt: int) -> float:
        retry_after = http_error.headers.get("Retry-After") if http_error.headers else None
        if retry_after:
            try:
                return max(0.0, min(float(retry_after), self.session_options.retry_max_wait))
            except ValueError:
                pass
        return min(self.session_options.retry_max_wait, self.session_options.retry_backoff * (2**attempt))
    ...
    def _request_json(self, path: str, params: Dict[str, Any], referer: str) -> Dict[str, Any]:
        ...
        for attempt in range(self.session_options.max_retries + 1):
            try:
                ...
                if exc.code == 429 and attempt < self.session_options.max_retries:
                    wait_seconds = self._compute_retry_wait(exc, attempt)
                    print(
                        f"[경고] 429(요청 제한) 감지: {wait_seconds:.1f}초 후 재시도 "
                        f"({attempt + 1}/{self.session_options.max_retries})",
                        file=sys.stderr,
                    )
                ...
                if exc.code == 429:
                    raise RuntimeError(
                        f"HTTP 429 오류 ({url})\n"
                        "요청이 너무 많아 서버에서 제한했습니다(TOO_MANY_REQUESTS).\n"
                        "--max-retries 값을 늘리거나 --retry-backoff / --retry-max-wait를 키워 재시도하세요.\n"
                        "또는 --cookie-file / --cookie / --bootstrap-browser-cookies 사용 후 요청 간격을 늘려보세요.\n"
                        f"응답: {body[:250]}"
                    ) from exc
        ...
        if last_http_error is not None:
            raise RuntimeError(f"HTTP 오류: {last_http_error}") from last_http_error
        raise RuntimeError("알 수 없는 네트워크 오류")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    search.add_argument("--max-retries", type=int, default=4, help="HTTP 429 재시도 횟수")
    search.add_argument("--retry-backoff", type=float, default=1.5, help="429 재시도 지수 백오프 시작값(초)")
    search.add_argument("--retry-max-wait", type=float, default=20.0, help="429 재시도 최대 대기시간(초)")
    ...
    crawl.add_argument("--max-retries", type=int, default=4, help="HTTP 429 재시도 횟수")
    crawl.add_argument("--retry-backoff", type=float, default=1.5, help="429 재시도 지수 백오프 시작값(초)")
    crawl.add_argument("--retry-max-wait", type=float, default=20.0, help="429 재시도 최대 대기시간(초)")
    ...
    crawl_from_search.add_argument("--max-retries", type=int, default=4, help="HTTP 429 재시도 횟수")
    crawl_from_search.add_argument("--retry-backoff", type=float, default=1.5, help="429 재시도 지수 백오프 시작값(초)")
    crawl_from_search.add_argument("--retry-max-wait", type=float, default=20.0, help="429 재시도 최대 대기시간(초)")
    ...
    try:
        collector = NaverLandCollector(
            session_options=SessionOptions(
                ...
                max_retries=max(0, int(getattr(args, "max_retries", 4))),
                retry_backoff=max(0.1, float(getattr(args, "retry_backoff", 1.5))),
                retry_max_wait=max(0.1, float(getattr(args, "retry_max_wait", 20.0))),
            )
        )
